...
class TextSR(base.TextBase):
    ...
    def test(self):
        with open(f'./{"baseline" if self.args.not_use_sr_method else self.args.arch}_result.txt', 'w') as f:
            model_dict = self.generator_init()
            model, image_crit = model_dict['model'], model_dict['crit']
            ...
            for test_dir in items:
                ...
                for i, data in (enumerate(test_loader)):
                    ...
                    images_hr = images_hr.to(self.device)
                    sr_beigin = time.time()
                    if self.args.not_use_sr_method:
                        images_sr = torch.nn.Upsample(scale_factor=2, mode="bilinear")(images_lr)
                    else:
                        images_sr = model(images_lr)
                    ...
                    for i_pred, (pred, target) in enumerate(zip(pred_str_sr, label_strs)):
                        lower_pred = str_filt(pred, 'lower')
                        lower_target = str_filt(target, 'lower')
                        if lower_target == lower_pred:
                            n_correct += 1
                        if len(lower_target) == 0:
                            continue
                        n_c_correct = 0
                        for i_c in range(min(len(lower_pred), len(lower_target))):
                            if lower_pred[i_c] == lower_target[i_c]:
                                n_c_correct += 1
                        f.write(f"{test_dir}_{i}_{i_pred},{'baseline' if self.args.not_use_sr_method else self.args.arch},{round(n_c_correct/len(lower_target), 4)},{len(lower_target)},{test_dir}\n")
                    sum_images += val_batch_size
                    torch.cuda.empty_cache()
                    if i % 10 == 0:
